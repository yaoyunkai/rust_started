...
**所有的 Copy 类型都天然是一个 Clone 类型。**

开发者通过创建“引用”这种数据类型，来完成“借用”这个动作。<br/>
Rust 编译器中有一个核心组件叫做“借用检查器（Borrow Checker）”，它的工作不是检查“引用”这个指针本身，而是检查你的“借用行为”是否符合安全规则
...
不会。借用时绝对不会发生所有权转移。 <br/>
借用的核心目的，正是为了在不获取、不改变所有权的前提下，允许程序的其他部分临时访问或修改数据。
